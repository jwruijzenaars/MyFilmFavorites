...
public class MovieUpcomingApiTask extends AsyncTask<String, Void, List<Movie>> {
    ...
    private List<Movie> doParseJsonToArrayList(String jsonString1) {
        ...
        try {
            ...
            for (int j = 0; j < movieList.length(); j++) {
                ...
                JSONObject resultsCast = null;
                if (CastSearchResults != null) {
                    resultsCast = new JSONObject(CastSearchResults);
                }
    ...
    private String getMovieIdFromJson (String jsonString) {
        ...
        try {
            ...
            JSONArray movieList = results.getJSONArray(JSON_MOVIE_GET_RESULTS);
            for (int j = 0; j < movieList.length(); j++) {
                JSONObject movie = new JSONObject(movieList.get(j).toString());
                id = movie.getString(JSON_MOVIE_ID);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
